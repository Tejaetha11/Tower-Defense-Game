...
class AbilityManager {
  ...
  update(dt) {
    ABILITY_DEFS.forEach(function(a) {
      ...
      if ((this._shake[a.id] || 0) > 0)
        this._shake[a.id] = Math.max(0, this._shake[a.id] - dt);
    }.bind(this));
  }
  ...
  handleClick(sx, sy, cssW, cssH) {
    ...
    for (var i = 0; i < ABILITY_DEFS.length; i++) {
      ...
      if (sx >= p.x && sx <= p.x + W && sy >= p.y && sy <= p.y + H) {
        ...
        if (this._coins !== undefined && this._coins < a.cost) {
          this._shake[a.id] = 0.45;
          return "card";
        }
  ...
  _drawCard(ctx, x, y, a, onCD, isSel, isHov, coins) {
    var W = ABILITY_CFG.cardW, H = ABILITY_CFG.cardH;
    var costSize = ABILITY_CFG.costSize;
    var frames   = _getCardFrames();

    // Shake offset
    var shakeT  = this._shake[a.id] || 0;
    var shakeOff = 0;
    if (shakeT > 0) {
      var freq = 28; // oscillations per second
      shakeOff = Math.sin(shakeT * freq) * 6 * (shakeT / 0.45);
    }
    x += shakeOff;
    ...
    // Cost
    var cantAfford = (!onCD && coins !== undefined && coins < a.cost);
    var coinX = x + W/2 - 13, coinY = y + H - 7;
    // Coin icon — always normal gold
    ...
    // Cost text — red when can't afford
    ctx.fillStyle = cantAfford ? "#ff4444" : (onCD ? "#6a5030" : "#3a1500");
  ...
  draw(ctx, cssW, cssH, coins) {
    this._coins = coins; // store so handleClick can access
    var positions = this._getPositions(cssW, cssH);
    for (var i = 0; i < ABILITY_DEFS.length; i++) {
      var a     = ABILITY_DEFS[i];
      var p     = positions[i];
      ...
      this._drawCard(ctx, p.x, p.y, a, onCD, isSel, isHov, coins);
    }
