...
class UpgradePanel {
  ...
  handleClick(sx, sy, towerManager, goldManager, scale) {
    ...
    if (sx >= this._btnX && sx <= this._btnX + this._btnW &&
      ...
      if (towerManager.getUpgradeCost(this.selectedTower) !== null) {
        towerManager.upgradeTower(this.selectedTower);
        this.show(this.selectedTower);
      }
      return true;
    }
    this.hide();
  ...
  draw(ctx, tower, scale, dpr) {
    ...
    const btnLabel  = isMax ? "✦ MAX LEVEL ✦" : `⬆ Upgrade  ${nextUp.cost} 🪙`;
    ...
    ctx.fillStyle = this._gradientV(
      ctx, btnX, btnY, btnH,
      isMax ? cfg.btnBgTopMax : cfg.btnBgTop,
      isMax ? cfg.btnBgBotMax : cfg.btnBgBot
    );
    ctx.fill();
    ctx.shadowColor = "transparent";

    this._roundRect(ctx, btnX, btnY, btnW, btnH, cfg.btnRadius);
    ctx.strokeStyle = isMax ? cfg.btnBorderMax : cfg.btnBorder;
    ctx.lineWidth   = 1.5;
    ctx.stroke();
    ...
    ctx.fillStyle    = isMax ? cfg.btnTextMax : cfg.btnTextColor;
    ctx.textAlign    = "center";
